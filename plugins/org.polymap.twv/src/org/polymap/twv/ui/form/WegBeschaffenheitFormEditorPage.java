...
public class WegBeschaffenheitFormEditorPage
    ...
    protected WegAbschnittBeschaffenheitComposite createNewComposite() {
        WegAbschnittBeschaffenheitComposite ret = TwvRepository.instance().newEntity( WegAbschnittBeschaffenheitComposite.class, null );
        ret.weg().set( weg );
        return ret;
    }
    ...
    public Control createBeschaffenheitForm( Composite parent, Composite top ) {
        ...
        Composite line2b = newFormField( "Bis Wegobjekt" )
                ...
                .setLayoutData( right().top( top ).create() ).create();

        Composite line3 = newFormField( IFormFieldLabel.NO_LABEL )
                .setToolTipText(
                        "Beschaffenheit aktueller Abschnitt z.B. naturnah, Asphalt, sandgeschlämmte Schotterdecke (technische Zustandsbeschreibung)" )
                .setParent( parent )
                .setEnabled( false )
                .setProperty(
                        new ReloadablePropertyAdapter<WegAbschnittBeschaffenheitComposite>( selectedComposite, prefix
                                + "name", new PropertyCallback<WegAbschnittBeschaffenheitComposite>() {

                            public Property get( WegAbschnittBeschaffenheitComposite entity ) {
                                return entity.name();
                            }
                        } ) )
                .setField(
                        reloadable( new TextFormFieldWithSuggestions( twvRepository.entitiesWithNames(
                                WegbeschaffenheitComposite.class ).keySet() ) ) )
